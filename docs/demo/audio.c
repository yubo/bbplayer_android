...
//	        LOGW("outputMixEnvironmentalReverb is NULL");
//	    }

//	    result = (*bqPlayerEffectSend)->EnableEffectSend(bqPlayerEffectSend,
//	            outputMixEnvironmentalReverb, (SLboolean) 1, (SLmillibel) 0);
//	    // and even if environmental reverb was present, it might no longer be available
//	    if (SL_RESULT_SUCCESS != result) {
//	         LOGW("bqPlayerEffectSend is NULL 0x%x",result);
//	    }
    ...
    if (nextSize > 0) {
        ...
        result = (*bqPlayerBufferQueue)->Enqueue(bqPlayerBufferQueue, nextBuffer, nextSize);
        if (SL_RESULT_SUCCESS != result) {
           LOGW("bqPlayerBufferQueue is NULL");
        }
    }
